...
class ValidationHelper {
     ...
     */
    static validUserRegister(cpf, password, name, email, lastName, address, civilStatus, cnpj, checkPassword = true, checkEmail = true) {
        let message;
        ...
        else if (!!civilStatus && !ValidationHelper.validCivilStatus(civilStatus))
            message = constants.invalidCivilStatus

        //campos opcionais endereço, valida caso venha preenchido.
        else if (!!address) {
            if (!!address.street && !ValidationHelper.validStreet(address.street))
                message = constants.invalidStreet
            else if (!!address.number && !ValidationHelper.validNumber(address.number))
                message = constants.invalidNumber
            else if (!!address.cep && !ValidationHelper.validNumber(address.cep))
                message = constants.invalidCEP
            else if (!!address.complement && !ValidationHelper.validComplement(address.complement))
                message = constants.invalidComplement
            else if (!!address.neighborhood && !ValidationHelper.validNeighborhood(address.neighborhood))
                message = constants.invalidNeighborhood
            else if (!!address.city && !ValidationHelper.validCity(address.city))
    ...
    static validReformRegister(userId, establishmentName, establishmentType, status, area, address, goal, restrictions, budgetLimit, reformItens, phone) {
        let message;

        if (!userId)
            message = constants.userIdNotFound
            ...
            message = constants.invalidReformItens
        else if (!reformItens || !ValidationHelper.validOutros(reformItens.outros))
