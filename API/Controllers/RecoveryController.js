...
class RecoveryController {
  ...
  static recoveryPassword(email, passCode, password, callback) {
    if (validPassword(password)) {
      let salt = RecoveryController.randomSHA256(constants.minRandomNumber, constants.maxRandomNumber)
      const hashedPassword = sha256(password + salt)
      DAO.updateUserByPassCode(email, passCode, hashedPassword, salt, callback)
    } else {
      const errorObj = { statusDesc: constants.invalidPassword, statusCode: constants.invalidFields }
